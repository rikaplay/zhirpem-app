  ...
  return (
    <main className="min-h-screen bg-background-light dark:bg-background-dark text-zinc-900 dark:text-zinc-100 pb-32">
      ...
      <header className="sticky top-0 z-30 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-xl px-4 py-4 flex items-center justify-between border-b border-zinc-500/5">
        <button onClick={() => setIsSidebarOpen(true)} className="w-11 h-11 glass rounded-2xl flex items-center justify-center overflow-hidden active:scale-90 transition-transform">
        ...
        <button onClick={() => setActiveBottomTab('notifications')} className="w-11 h-11 glass rounded-2xl flex items-center justify-center relative active:scale-90 transition-transform">
      ...
      <div className="max-w-[500px] mx-auto min-h-screen">
        ...
        {activeBottomTab === 'profile' && <UserProfileScreen username={profileUsername || session.username} myUser={userObj} onBack={() => setActiveBottomTab('home')} />}
      ...
      {!isSidebarOpen && !activeChatId && <BottomNav activeTab={activeBottomTab} onTabChange={handleTabChange} />}
    </main>
  );
}
