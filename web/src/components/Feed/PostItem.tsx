...
export const PostItem: React.FC<PostItemProps> = ({
  ...
  const [isLiked, setIsLiked] = useState(post.likedBy.includes(myUsername));
  const [isBookmarked, setIsBookmarked] = useState(post.bookmarkedBy.includes(myUsername));
  const [isReposted, setIsReposted] = useState(post.repostedBy.includes(myUsername));
      ...
      case MediaType.IMAGE:
        return (
          <img
            ...
            onClick={(e) => { e.stopPropagation(); /* Fullscreen logic */ }}
          />
        );
      case MediaType.GIF:
        return (
          <img
            src={url}
            alt="GIF"
            className="w-full rounded-2xl object-cover max-h-[500px] mt-2"
          />
        );
  ...
  return (
    ...
    >
            ...
            <div className="flex flex-col" onClick={(e) => { e.stopPropagation(); onUserClick(post.handle.replace('@', '')); }}>
              <div className="flex items-center gap-1">
                <span
                  ...
                  style={{ color: post.authorNameColor || 'inherit' }}
                >
                  {post.author}
                </span>
              ...
              {post.authorStatus && (
                <span className="text-primary text-[12px] font-semibold leading-tight">
                  {post.authorStatus}
                </span>
              )}
            </div>
      ...
      <div className="flex items-center gap-6 mt-4">
        <button
          className={`flex items-center gap-1.5 py-1 px-2 rounded-xl transition-all active:scale-90 ${post.commentsCount > 0 ? 'text-primary' : 'text-zinc-500'}`}
          onClick={(e) => { e.stopPropagation(); /* Comments toggle */ }}
        >
          <span className="text-sm font-medium">
            {post.commentsCount > 0 ? `Ответить(${post.commentsCount})` : 'Ответить'}
          </span>
        </button>

        <button
          className={`flex items-center gap-1.5 py-1 px-2 rounded-xl transition-all active:scale-90 ${isLiked ? 'text-pink-500' : 'text-zinc-500'}`}
          onClick={handleLike}
        >
      ...
      <div className="flex items-center justify-between">
        <button
          className={`flex items-center gap-1.5 py-1 px-2 rounded-xl transition-all active:scale-90 ${isBookmarked ? 'text-primary' : 'text-zinc-400'}`}
        ...
        <div className="flex items-center gap-3">
          <button
            className={`flex items-center gap-1.5 py-1 px-2 rounded-xl transition-all active:scale-90 ${isReposted ? 'text-green-500' : 'text-zinc-400'}`}
